...
\begin{array}{c|cccc}
|G| & 4 & 5 & 6 & 7\\ \hline
\max_h |\{g\in G:h\in g\}| & \geq2 & \geq3 & \geq4 & \geq4.
\end{array}
\]
...
the other three cases we use a finite incidence-pattern reduction, which is
also the intended Lean proof route.
...
Let $B,C,D,T$ be the $110$, $001$, $011$, and $111$ fibers respectively.
Every finite union-closed fiber has a maximum auxiliary set, so bottom-fiber
propagation already shows that $h$ appears in the maxima of all four nonzero
fibers.  If $h$ appears in every member of $B$, then $h$ appears in at least
\[
  2+k+1+1+1=k+5
\]
members of the original family.

Otherwise choose $b\in B$ with $h\notin b$.  Since $110\vee001=111$, for
every $c\in C$ the union $b\cup c$ is the unique member of the $111$ fiber;
therefore $h\in c$.  Similarly, since $110\vee011=111$, every member of
$D$ contains $h$.  Together with one occurrence in the maximum of $B$, this
gives at least
\[
  2+1+k+2+1=k+6
\]
occurrences.  In either case $h$ appears at least $k+5$ times, while
\[
...
At total size $17$, the same scan reports the first unpaid classes: $66$
sign-separated trace counts, of which $48$ are paid by propagation and four
canonical classes remain unpaid.  Three of these have zero-fiber size $8$,
where the local zero-fiber bound has not yet been added.  The remaining class
\[
  (6,0,0,3,3,0,3,2)
\]
has propagation lower bound $8$, exactly one occurrence short of half.  This
is the current next obstruction layer.
...
\begin{enumerate}
  ...
  fiber.
  \item Small local Frankl bounds for zero fibers of sizes $4$, $5$, $6$, and
  $7$.
...
The Lean project currently proves the singleton-containing case.  The next
formal targets should be low-risk statements that clarify the attack:
\begin{enumerate}
