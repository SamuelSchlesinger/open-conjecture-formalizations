---
title: Combinatorics
layout: default
parent: Open Problems
nav_order: 6
---

# Combinatorics & Order Theory — Open Conjectures

[← Back to Open Problems](README.md) · [← Back to Index](../README.md)

## Combinatorics

| # | Conjecture | Eponym(s) | Comments |
|---|-----------|-----------|----------|
| 1 | [Dittert conjecture](https://en.wikipedia.org/wiki/Dittert_conjecture) | Eric Dittert | |
| 2 | [Frankl conjecture](https://en.wikipedia.org/wiki/Frankl_conjecture) | Péter Frankl | |
| 3 | [Hadamard conjecture](https://en.wikipedia.org/wiki/Hadamard_conjecture) | Jacques Hadamard | |

## Order Theory

| # | Conjecture | Eponym(s) | Comments |
|---|-----------|-----------|----------|
| 1 | [1/3–2/3 conjecture](https://en.wikipedia.org/wiki/1/3%E2%80%932/3_conjecture) | Kislitsyn; Fredman; Linial | [In-repo formalization](../order-theory/one-third-two-thirds/): statement + linear-extension counting, tightness of 1/3, duality & twin reductions proved; conjecture open |
| 2 | [Gold partition conjecture](https://en.wikipedia.org/wiki/1/3%E2%80%932/3_conjecture#Generalizations_and_related_results) | — | |

## Additive Combinatorics

| # | Conjecture | Eponym(s) | Comments |
|---|-----------|-----------|----------|
| 1 | [Rudin's conjecture](https://en.wikipedia.org/wiki/Rudin%27s_conjecture) | Walter Rudin | |

## Other

| # | Conjecture | Field | Eponym(s) | Comments |
|---|-----------|-------|-----------|----------|
| 1 | [Scholz conjecture](https://en.wikipedia.org/wiki/Scholz_conjecture) | Addition chains | Arnold Scholz | |
| 2 | [Singmaster's conjecture](https://en.wikipedia.org/wiki/Singmaster%27s_conjecture) | Binomial coefficients | David Singmaster | [In-repo formalization](../combinatorics/singmaster/): computable multiplicity, faithful counter; `N(a)≥2`, an infinite `N≥4` family, and `N(3003)≥8` proved (so `C≥8`, the conjectured value); uniform bound open |

## References

- van Lint, J. H. & Wilson, R. M. *A Course in Combinatorics*. 2nd ed., Cambridge University Press, 2001.
- Stanley, R. P. *Enumerative Combinatorics*. Vol. 1 & 2, Cambridge University Press.

For references on the formalized Dittert, Frankl, Gold partition, Hadamard, and Sensitivity conjectures, see the [centralized bibliography](../references.md#combinatorics).
